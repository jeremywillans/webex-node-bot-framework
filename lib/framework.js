...
Framework.prototype.log = function (message) {
   ...
   * log messages from the framework, such as details about events that were
        ...
        return this.webex.memberships.list({roomId: roomId})
          .then((m) => {
            if (m.length > 1) {
              ...
              // Attempt to remove bot membership from space and remove this space from "in spawning process" array
              this.roomsSpawningNow = _.remove(this.roomsSpawningNow, roomId);
              this.webex.memberships.remove(m.items[0].id)
              .then((m) => {
                console.log(m);
                // Reject indicating removal of membership
                return when.reject(new Error(`Bot membership removed from 1-1 space where bot is the only remaining member.`));
              })
              .catch((error) => {
                // Reject indicating not spawning in 1-1 space
                console.log(error);
